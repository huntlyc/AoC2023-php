...
foreach ($lines as $line) {
    ...
    $nums[0][] = 0; // next num in zero arr always zero
    for($i = 1; $i < count($nums); $i++){

        // next number in array is difference is last num in cur array + last num in last arr
        $lastCur = $nums[$i][count($nums[$i]) - 1];
        $lastPrev = $nums[$i - 1][count($nums[$i - 1]) - 1];

        $nums[$i][] = $lastCur + $lastPrev;
    }

    $lastA = $nums[count($nums)-1];
    $sum += $lastA[count($lastA)-1]; 
}
