...
$input = file_get_contents(DEBUG ? $testInputs[3] : 'input.txt');
...
do{
    ...
    //echo "at end? {$curPos[0]},{$curPos[1]} !== {$startPos[0]},{$startPos[1]}\n";
//}while($steps < 3);
}while(count($steps) < 20000 && "{$curPos[0]},{$curPos[1]}" !== "{$startPos[0]},{$startPos[1]}");

$maxDist = ceil(count($steps) / 2);
echo $maxDist . PHP_EOL;
echo array_pop($steps) . PHP_EOL;
